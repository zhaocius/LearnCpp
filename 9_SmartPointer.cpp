...
void unique_ptr_test()
{
    ...
    std::unique_ptr<Malloc_Test> up4 = std::make_unique<Malloc_Test>(1, 2); //key 使用make_unique代替new
    ...
    std::unique_ptr<Malloc_Test> up2(up.release()); //key 将up释放，并赋给up3。
...
//    std::unique_ptr<Malloc_Test>up5 = up; //key 不支持赋值。
    ...
    std::unique_ptr<Malloc_Test[]> tests(new Malloc_Test[3]);
...
{
    Malloc_Test *test = new Malloc_Test();
    std::shared_ptr<Malloc_Test> sp(test);  //绑定已有的指针。最好不好混合使用普通指针和智能指针。
